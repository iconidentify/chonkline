//! Self-contained cryptographic primitives used by account authentication
//! (PBKDF2 password hashing, SASL PLAIN) and host cloaking (HMAC-SHA256).
//!
//! Hand-rolled to keep the server dependency-light (std + tokio only) and to
//! avoid any C toolchain in the container build. Every routine is checked
//! against published NIST / RFC test vectors in the unit tests below.
...
pub fn sha256(msg: &[u8]) -> [u8; 32] {
    ...
    // Pad: message || 0x80 || 0x00... || 64-bit big-endian length.
    let mut data = msg.to_vec();
    data.push(0x80);
    while data.len() % SHA256_BLOCK != 56 {
        data.push(0);
    }
    data.extend_from_slice(&ml_bits.to_be_bytes());

    let mut w = [0u32; 64];
    for block in data.chunks_exact(SHA256_BLOCK) {
        for i in 0..16 {
...
pub fn hmac_sha256(key: &[u8], msg: &[u8]) -> [u8; 32] {
    ...
    if key.len() > SHA256_BLOCK {
        k[..SHA256_OUT].copy_from_slice(&sha256(key));
    } else {
    ...
    let mut outer = Vec::with_capacity(SHA256_BLOCK + SHA256_OUT);
...
pub fn pbkdf2_sha256(pass: &[u8], salt: &[u8], iters: u32, dk_len: usize) -> Vec<u8> {
    let mut out = Vec::with_capacity(dk_len);
    let mut block_index: u32 = 1;
    while out.len() < dk_len {
        // U1 = PRF(pass, salt || INT(block_index))
...
/// Constant-time byte-slice equality (defends password/tag comparison against
/// timing analysis). Returns false immediately on a length mismatch.
...
mod tests {
    ...
    fn pbkdf2_sha256_known_vectors() {
        assert_eq!(
            hex(&pbkdf2_sha256(b"password", b"salt", 1, 32)),
    ...
    fn base64_roundtrip_and_vectors() {
        ...
        // SASL PLAIN payload: authzid \0 authcid \0 passwd
