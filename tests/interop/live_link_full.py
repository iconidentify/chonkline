...
INSP_CLIENT, INSP_LINK, CHONK_CLIENT = 7000, 7001, 7100
...
try:
    ...
    env = dict(os.environ, IRC_PORT=str(CHONK_CLIENT), IRC_SID="2CH",
               ...
               IRC_LINK_PEERS=f"127.0.0.1:{INSP_LINK}", IRC_LOG_LEVEL="info", IRC_HTTP_PORT="0")
    ...
    # --- quit propagates ---
    cc.send("JOIN #bridge"); time.sleep(2)
    ci.clear()
...
order = ["link_up","insp_sees_remote_join","chan_chonk_to_insp","chan_insp_to_chonk",
         ...
         "quit_propagates","split_detected","chonkline_survives_split","usable_after_split"]
for k in order:
